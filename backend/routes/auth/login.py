...
def login():
    ...
    try:
        mdp = fetch_one("SELECT id, mdp, is_admin, is_ban FROM users WHERE email = %s", (email,))
        if mdp is None:
            return jsonify({"status": "error"}), 403

        if int(bool(mdp.get("is_ban", 0))) == 1:
        ...
        if not checkpw(motdepasse_bytes, hash_bytes):
            ...
            admin_flag = int(bool(mdp.get("is_admin", 0)))

            if check_a2f_status(mdp["id"]) == 2:
                token = encode_jwt({"id": mdp["id"], "a2f": 1, "admin": admin_flag, "is_admin": admin_flag}, expires_in=3600)
                message = "Login successful with 2FA wait a2f verification"
            else:
                token = encode_jwt({"id": mdp["id"], "a2f": 0, "admin": admin_flag, "is_admin": admin_flag}, expires_in=3600)
